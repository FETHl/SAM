...
SCRIPT_DATE = "2025-03-23 18:56:39"
SCRIPT_AUTHOR = "FETHI"
...
class EnhancedCRFProcessor:
    """Advanced class for refining masks with Conditional Random Fields"""
    ...
    def __init__(self):
        self.default_params = {
            ...
            'edge_factor': 0.8,        # Edge weight factor
            'detail_level': 'high',    # Detail preservation level: 'low', 'medium', 'high'
            'use_edge_enhancement': True  # Whether to use edge detection for enhancement
        }
    
    def apply_crf(self, image, mask, params=None):
        """
        Refine mask using DenseCRF with enhanced edge preservation
        ...
        try:
            ...
            # Adaptive parameter tuning based on object size and detail
            mask_area_ratio = np.sum(mask) / (h * w)
            
            # Adjust parameters based on object size
            adjusted_params = params.copy()
            if mask_area_ratio < 0.05:  # Small object
                adjusted_params['bilateral_srgb'] = max(8, params['bilateral_srgb'] * 0.8)
                adjusted_params['bilateral_sxy'] = max(50, params['bilateral_sxy'] * 0.8)
            elif mask_area_ratio > 0.3:  # Large object
                adjusted_params['bilateral_srgb'] = min(18, params['bilateral_srgb'] * 1.2)
                adjusted_params['bilateral_sxy'] = min(100, params['bilateral_sxy'] * 1.2)
            
            # Edge-aware refinement
            if adjusted_params.get('use_edge_enhancement', True):
                ...
                # Use edge information to adjust CRF parameters locally
                edge_factor = adjusted_params.get('edge_factor', 0.8)
                
                # Calculate mask boundary
                mask_uint8 = mask.astype(np.uint8)
                kernel = np.ones((3, 3), np.uint8)
                mask_boundary = cv2.dilate(mask_uint8, kernel) - cv2.erode(mask_uint8, kernel)
                
                # Check if mask boundary overlaps with image edges
                edge_overlap = np.sum(edges * mask_boundary) / max(1, np.sum(mask_boundary))
                
                # If good edge overlap, increase edge sensitivity
                if edge_overlap > 0.2:
                    adjusted_params['bilateral_compat'] = max(5, adjusted_params['bilateral_compat'] * edge_factor)
            ...
            # Adjust confidence based on detail level
            detail_level = adjusted_params.get('detail_level', 'high')
            if detail_level == 'high':
                fg_prob, bg_prob = 0.9, 0.9
            elif detail_level == 'medium':
                fg_prob, bg_prob = 0.8, 0.8
            else:
                fg_prob, bg_prob = 0.7, 0.7
                
            U = np.zeros((2, h, w), dtype=np.float32)
            
            # Foreground probability
            U[0, mask_uint8 == 0] = bg_prob  # Background probability where mask is 0
            U[0, mask_uint8 == 1] = 1 - bg_prob
            U[1, mask_uint8 == 1] = fg_prob  # Foreground probability where mask is 1
            U[1, mask_uint8 == 0] = 1 - fg_prob
            ...
            d.addPairwiseGaussian(
                sxy=(adjusted_params['gaussian_sxy'], adjusted_params['gaussian_sxy']),
                compat=adjusted_params['gaussian_compat'],
            ...
            d.addPairwiseBilateral(
                sxy=(adjusted_params['bilateral_sxy'], adjusted_params['bilateral_sxy']),
                srgb=(adjusted_params['bilateral_srgb'], adjusted_params['bilateral_srgb'], adjusted_params['bilateral_srgb']),
                rgbim=image,
                compat=adjusted_params['bilateral_compat'],
                kernel=dcrf.DIAG_KERNEL,
                normalization=dcrf.NORMALIZE_SYMMETRIC
            )
            
            # Perform inference
            Q = d.inference(adjusted_params['iterations'])
            
            # Get the most probable class for each pixel
            MAP = np.argmax(Q, axis=0).reshape((h, w))
            
            # Apply post-processing to clean up the result
            result = MAP.astype(np.uint8)
...
class AdvancedMaskRefinement:
    ...
    def refine_mask(image, mask, params=None):
        ...
        if params.get('contour_smoothing', True):
            # Extract contours - this line was incomplete
...
def refine_mask_with_u2net(image_rgb, mask, u2net_predictor, multiscale=True):
    """
    Enhanced mask refinement using U²-Net with multi-scale prediction and smart fusion
    ...
    Args:
        ...
        multiscale: Whether to use multi-scale prediction for better results
    ...
    try:
        ...
        # Create an adaptive context region - larger for smaller objects
        # Calculate object size relative to image
        relative_size = np.sum(mask_uint8) / (mask_uint8.shape[0] * mask_uint8.shape[1])
        
        # Determine context border size - larger padding for smaller objects
        if relative_size < 0.05:  # Small object
            context_padding = 100  # Larger context for small objects
        elif relative_size < 0.2:  # Medium object
            context_padding = 70
        else:  # Large object
            context_padding = 50
            
        # Create context mask with adaptive padding
        kernel = np.ones((context_padding, context_padding), np.uint8)
        dilated_mask = cv2.dilate(mask_uint8, kernel, iterations=1)
        
        # Get bounding box of dilated mask for ROI extraction
        y_indices, x_indices = np.where(dilated_mask > 0)
        
        if len(y_indices) > 0 and len(x_indices) > 0:
            # Get bounds with padding
            x_min = max(0, np.min(x_indices) - 10)
            y_min = max(0, np.min(y_indices) - 10)
            x_max = min(image_rgb.shape[1], np.max(x_indices) + 10)
            y_max = min(image_rgb.shape[0], np.max(y_indices) + 10)
            
            # Extract ROI for more efficient processing
            roi = image_rgb[y_min:y_max, x_min:x_max]
            roi_mask = mask_uint8[y_min:y_max, x_min:x_max]
            roi_dilated = dilated_mask[y_min:y_max, x_min:x_max]
            
            # Create focused image for U²-Net
            context_region = roi.copy()
            
            # Use soft masking instead of hard cutoff for better boundaries
            # Outside the dilated region, gradually fade to black
            fade_mask = cv2.distanceTransform(1 - roi_dilated, cv2.DIST_L2, 5)
            fade_mask = np.clip(fade_mask, 0, 30) / 30.0
            fade_mask = 1.0 - fade_mask
            fade_mask = np.expand_dims(fade_mask, axis=2)
            
            # Apply soft masking - fade to dark outside ROI
            context_region = (context_region * fade_mask).astype(np.uint8)
            
            # Step 2: Use U²-Net for prediction with optional multi-scale
            if multiscale:
                # Use multiple scales for better detail preservation
                scales = [256, 320, 384]
                u2net_preds = []
                
                for size in scales:
                    pred = u2net_predictor.predict(context_region, size=size)
                    if pred is not None:
                        # Resize back to ROI size
                        pred_resized = cv2.resize(pred, (roi.shape[1], roi.shape[0]))
                        u2net_preds.append(pred_resized)
                
                # Combine predictions from different scales with weighted averaging
                if u2net_preds:
                    # Weight larger scales more for detail preservation
                    weights = np.array([0.2, 0.3, 0.5])[:len(u2net_preds)]
                    weights = weights / weights.sum()  # Normalize weights
                    
                    u2net_mask = np.zeros_like(u2net_preds[0])
                    for i, pred in enumerate(u2net_preds):
                        u2net_mask += pred * weights[i]
                else:
            ...
            # Step 3: Combine original mask with U²-Net mask
            # Normalize the U²-Net probability map with improved normalization
            u2net_mask = (u2net_mask - u2net_mask.min()) / (u2net_mask.max() - u2net_mask.min() + 1e-8)
            
            # Use adaptive thresholding for more reliable binary mask generation
            # Generate binary mask using Otsu's method for adaptive threshold
            if np.max(u2net_mask) > 0:
                u2net_mask_uint8 = (u2net_mask * 255).astype(np.uint8)
                _, binary_u2net = cv2.threshold(u2net_mask_uint8, 0, 1, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
            else:
                # Fallback to fixed threshold
                binary_u2net = (u2net_mask > 0.5).astype(np.uint8)
            
            # Step 4: Smart fusion of SAM and U²-Net masks
            # Use SAM for structure, U²-Net for details
            structure_weight = 0.7  # Weight for original SAM mask structure
            detail_weight = 0.3     # Weight for U²-Net details
            
            # Ensure binary_u2net is constrained to dilated region
            binary_u2net = binary_u2net & roi_dilated
            
            # Get boundaries of both masks for detail fusion
            sam_boundary = cv2.dilate(roi_mask, np.ones((3,3), np.uint8)) - cv2.erode(roi_mask, np.ones((5,5), np.uint8))
            u2net_boundary = cv2.dilate(binary_u2net, np.ones((3,3), np.uint8)) - cv2.erode(binary_u2net, np.ones((3,3), np.uint8))
            
            # Create fusion mask:
            # - Keep SAM structure in non-boundary regions
            # - Use weighted combination in boundary regions
            fusion_mask = roi_mask.copy()
            boundary_region = (sam_boundary | u2net_boundary) > 0
            
            # In boundary regions, use weighted average
            if np.any(boundary_region):
                fusion_mask[boundary_region] = ((roi_mask[boundary_region].astype(float) * structure_weight + 
                                               binary_u2net[boundary_region].astype(float) * detail_weight) > 0.5).astype(np.uint8)
            
            # Step 5: Clean up the mask
            # Remove small holes
            if np.sum(fusion_mask) > 100:  # Only process if mask has reasonable size
                fusion_mask = cv2.morphologyEx(fusion_mask, cv2.MORPH_CLOSE, np.ones((5, 5), np.uint8))
            
            # Remove small disconnected regions
            # Find connected components
            num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(fusion_mask, connectivity=8)
            ...
            if num_labels > 1:
                ...
                refined_roi_mask = (labels == largest_label).astype(np.uint8)
            else:
                refined_roi_mask = fusion_mask
                
            # Create full-size output mask
            refined_mask = mask.copy()
            refined_mask[y_min:y_max, x_min:x_max] = refined_roi_mask
            
            return refined_mask
        else:
            return mask  # Return original if no valid ROI
...
def create_segments_for_svg_export(masks, labels, image_shape, contour_method='skimage', 
    ...
    """
    Convert masks to a format suitable for SVGExporter with consistent colors for the same classes
    
    This function prepares segmentation data for SVG export, ensuring each class gets
    a consistent color across all instances, with detailed contours and metadata.
    ...
    # Assign colors to classes instead of individual objects
    for i, class_name in enumerate(sorted(set(class_names))):
        if i < len(colors):
            # Use predefined colors for common classes
            class_colors[class_name] = colors[i]
        else:
            # Generate color using golden ratio for distributing colors evenly
    ...
    for i, (mask, label) in enumerate(zip(masks, labels)):
        ...
        # Get the color assigned to this class
        ...
        segment = {
            ...
            # Include class name in object ID for better grouping
...
def save_mask_as_png(mask, output_path, class_name, class_colors):
    """
    Save an individual mask as a transparent PNG with class-consistent colored overlay.
    ...
    if class_name in class_colors:
        ...
        color_rgb = (0, 0, 255)  # Default to blue
...
def apply_crf(image, mask, crf_params):
    ...
    try:
        ...
        crf_processor = EnhancedCRFProcessor()
...
def process_pipeline(model,
    ...
    for i, mask in enumerate(all_masks):
        ...
        if u2net_predictor is not None and u2net_predictor.model is not None:
            ...
            processed_mask = refine_mask_with_u2net(image_rgb, processed_mask, u2net_predictor, 
                                                  multiscale=kwargs.get('multiscale_sam', True))
    ...
    if export_png and refined_masks:
        ...
        # Extract class names from labels for consistent coloring
        class_names = [label.split()[0] for label in labels]
        
        # Generate colors for classes (not individual objects)
        class_colors = {}
        colors = [
            (0.9, 0.1, 0.1),   # Red
            (0.1, 0.6, 0.9),   # Blue
        ...
        # Assign colors to unique class names
        for i, class_name in enumerate(sorted(set(class_names))):
            if i < len(colors):
                class_colors[class_name] = colors[i]
            else:
                # Generate color using golden ratio for even distribution
        ...
        for i, (mask, label) in enumerate(zip(refined_masks, labels)):
            ...
            save_mask_as_png(mask, png_path, class_name, class_colors)
...
def inference_detector(model,
                       ...
                       contour_method='both',  # Default to 'both' for better results
                       precision_level=6,
                       ...
                       detailed_refinement=True,
                       edge_refinement=True,
                       expand_box_ratio=0.15,  # Increased to capture more context
                       multiscale_sam=True,
                       svg_quality='professional',
                       verify_export=False,
    ...
    if all_masks:
        ...
        for i, mask in enumerate(all_masks):
            ...
            if u2net_predictor is not None and u2net_predictor.model is not None:
                print(f"  Applying U²-Net refinement...")
                processed_mask = refine_mask_with_u2net(image_rgb, processed_mask, u2net_predictor, multiscale=multiscale_sam)
    ...
    if export_svg and refined_masks:
        ...
        # Prepare segments data for SVG export - with per-class unique colors
        segments_data = create_segments_for_svg_export(
            refined_masks,
            labels,
            image.shape[:2],
        ...
        if segments_data:
            ...
            if success:
                ...
                if verify_export and len(refined_masks) > 0:
                    verification_image_path = os.path.join(vis_dir, f"{name_without_ext}_svg_verification.png")
                    metrics = verify_svg_export(svg_path, refined_masks[0], verification_image_path)
    ...
    if export_png and refined_masks:
        ...
        # Extract class names for consistent coloring
        class_names = [label.split()[0] for label in labels]
        
        # Generate class colors dictionary (same as in SVG export)
        class_colors = {}
        for i, class_name in enumerate(sorted(set(class_names))):
            # Use same color assignment logic as in create_segments_for_svg_export
            if i < len(colors := [
                (0.9, 0.1, 0.1),   # Red
                (0.1, 0.6, 0.9),   # Blue
                (0.2, 0.7, 0.2),   # Green
...
def main():
    ...
    total_objects = sum(1 for result in results if result for _ in range(getattr(result, 'detection_count', 0)))
    print(f"Total objects detected: {total_objects}")
    
    # Statistics by class
    class_counts = {}
    for result in results:
        if result and hasattr(result, 'detection_labels'):
...
if __name__ == '__main__':
    ...
    tokenizer = AutoTokenizer.from_pretrained("openai/clip-vit-base-patch32", cache_dir="./pretrained_models")
    print("CLIP tokenizer téléchargé avec succès !")
    main()
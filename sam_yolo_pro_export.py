#!/usr/bin/env python3
"""
YOLO-World + SAM + U²-Net Professional SVG Exporter
Integrates YOLO-World for object detection, SAM for segmentation, 
and U²-Net for mask refinement with SVG export capabilities.
...
SCRIPT_DATE = "2025-03-21 12:30:00"
SCRIPT_AUTHOR = "FETHI"

print(f"YOLO-World + SAM + U²-Net Professional SVG Exporter v{SCRIPT_VERSION}")
...
class SVGExporter:
    ...
    def create_svg_document(self, size, metadata=None):
        ...
        # Add style definitions for professional and maximum quality levels
        if self.quality_level in ["professional", "maximum"]:
            # Add stylesheet
            ...
            # CSS rules
            css = """
                .mask { fill-opacity: 0.5; stroke-width: 1; stroke-opacity: 0.8; }
                .bbox { fill: none; stroke-width: 1; stroke-dasharray: 5,5; opacity: 0.7; }
    ...
    def add_shape_to_svg(self, svg_root, element, color=None, segment_id=None):
        ...
        elif element_type == 'rectangle' or element_type == 'polygon' or element_type == 'polygone' or element_type == 'contour':
        ...
        if shape_elem is not None:
            ...
            # Add class for CSS styling in professional and maximum quality
            if self.quality_level in ["professional", "maximum"]:
            ...
            for idx, segment in enumerate(segments_data):
                ...
                for element in segment['geometric_elements']:
                    ...
                    if element['type'] == 'contour' and 'points' in element:
                        ...
                        # Debug output for contour points in maximum mode
                        if self.quality_level == "maximum":
                            print(f"  Contour in segment {segment_id} has {len(element['points'])} points")
                        valid_contour_count += 1
                        segment_has_valid_contours = True
                        
                    self.add_shape_to_svg(group, element, color, segment_id)
                
                # If no valid contours in this segment, add a simple shape
                if not segment_has_valid_contours and 'class' in segment:
                    # Create a simple rectangle with the class name as text
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='YOLO-World with SAM, U²-Net and Professional SVG Export')
...
# Replace the initialize_sam function with this improved version:
...
# Replace the enhanced_sam_inference function with this simpler, more robust version:
def enhanced_sam_inference(sam_predictor, image_rgb, box, multimask_output=True):
    """Perform SAM inference with better error handling."""
    if sam_predictor is None:
        return None
    
    try:
        # Ensure box is properly formatted with floating point values
        box = [float(coord) for coord in box]
        box_torch = torch.tensor([box], dtype=torch.float, device=sam_predictor.device)
        
        # Run SAM prediction
        masks, _, _ = sam_predictor.predict(
            point_coords=None,
            point_labels=None,
            box=box_torch[0],  # Pass as a single box, not batched
            multimask_output=multimask_output
        )
        
        # Return the best mask (first one if multiple)
        if multimask_output and masks.shape[0] > 0:
            return masks[0]
        return masks
...
# In the inference_detector function, modify this part:
# After setting the image with sam_predictor.set_image(image_rgb)
for i, box_prompt in enumerate(box_prompts):
    print(f"Processing object {i+1}/{len(box_prompts)}: {labels[i]}")
    
    try:
        # Ensure box is valid
        x1, y1, x2, y2 = box_prompt
        if x2 <= x1 or y2 <= y1 or x1 >= image_rgb.shape[1] or y1 >= image_rgb.shape[0]:
            print(f"  ⚠️ Invalid box dimensions: {box_prompt}")
            box_mask = np.zeros(image_rgb.shape[:2], dtype=np.uint8)
            all_masks.append(box_mask)
            continue
            
        # Simple approach - get masks directly without complex transformations
        mask = enhanced_sam_inference(
            sam_predictor, 
            image_rgb, 
            box_prompt,
        ...
        if mask is not None and mask.sum() > 0:
            ...
            print(f"  ⚠️ SAM failed to generate mask, using box mask instead")
            box_mask = np.zeros(image_rgb.shape[:2], dtype=np.uint8)
...
def apply_crf(image, mask, crf_params):
    """
    Refine a binary mask using Conditional Random Fields.
    """
    try:
        # Get parameters
        pos_w = crf_params.get('pos_w', 5.0)
        bi_w = crf_params.get('bi_w', 10.0)
        bi_xy = crf_params.get('bi_xy', 60.0)
        bi_rgb = crf_params.get('bi_rgb', 10.0)
        iterations = crf_params.get('iter', 15)
        
        # Create CRF model
        height, width = image.shape[:2]
        d = dcrf.DenseCRF2D(width, height, 2)  # 2 classes: foreground and background
        
        # Create class probability maps (softmax)
        probs = np.zeros((2, height, width), dtype=np.float32)
        
        # Ensure mask is uint8 format
        mask_uint8 = mask.astype(np.uint8)
        
        # Set probabilities based on the mask
        probs[0, mask_uint8 == 0] = 0.9  # Background
        probs[0, mask_uint8 == 1] = 0.1
        probs[1, mask_uint8 == 1] = 0.9  # Foreground
        probs[1, mask_uint8 == 0] = 0.1
        
        # Set unary potentials from our probability maps
        U = unary_from_softmax(probs)  
        d.setUnaryEnergy(U)
        
        # Add pairwise potentials
        d.addPairwiseGaussian(sxy=pos_w, compat=3)
        d.addPairwiseBilateral(sxy=bi_xy, srgb=bi_rgb, rgbim=image, compat=bi_w)
        
        # Run inference
        Q = d.inference(iterations)
        MAP = np.argmax(Q, axis=0).reshape((height, width))
        
        # The result is a segmentation where 1 indicates foreground
        refined_mask = (MAP == 1).astype(np.uint8)
...
def optimized_mask_refinement(image_rgb, mask, refinement_mode='high_quality'):
    ...
    # Define refinement parameters based on mode
    ...
    # Step 1: Apply CRF refinement
    refined_mask = apply_crf(image_rgb, mask_uint8, crf_params)
    
    # Step 2: Apply detailed refinement with edge awareness
    refined_mask = AdvancedMaskRefinement.refine_mask(image_rgb, refined_mask, detail_params)
    
    # Step 3: Clean up mask with conservative operations to preserve details
    # Minimal filtering to maintain edges while removing noise
    kernel = np.ones((3, 3), np.uint8)
...
def create_segments_for_svg_export(masks, labels, image_shape, contour_method='skimage', 
                                  simplify_tolerance=0.1, svg_quality='standard'):
    """
    Convert masks to a format suitable for SVGExporter
    ...
    Args:
        ...
        simplify_tolerance: Tolerance for simplifying contours
        svg_quality: Quality level for SVG export
    ...
    color_map = {
        'person': (0.9, 0.1, 0.1),      # Red 
        'table': (0.1, 0.6, 0.9),       # Blue
        'chair': (0.2, 0.7, 0.2),       # Green
        'car': (0.8, 0.2, 0.8),         # Purple
        'dog': (1.0, 0.6, 0.0),         # Orange
        'cat': (0.8, 0.8, 0.2),         # Yellow
        'bottle': (0.0, 0.6, 0.6),      # Teal
        'cup': (0.6, 0.0, 0.6),         # Magenta
        'book': (0.5, 0.5, 0.5),        # Gray
        'phone': (0.8, 0.4, 0.0),       # Brown
        'laptop': (0.0, 0.4, 0.8),      # Royal Blue
        'keyboard': (0.4, 0.4, 0.4),    # Dark Gray
        'tv': (0.2, 0.2, 0.6),          # Navy
        'mouse': (0.6, 0.6, 0.2),       # Olive
        'remote': (0.6, 0.2, 0.0),      # Rust
        'couch': (0.1, 0.5, 0.3),       # Forest Green
        'dining table': (0.1, 0.6, 0.9), # Blue
        'ladle': (0.7, 0.3, 0.5),       # Mauve
        'glove': (0.3, 0.7, 0.9),       # Sky Blue
        'mold': (0.5, 0.2, 0.8),        # Purple
        'mask': (0.3, 0.8, 0.7),        # Teal
        'glasses': (0.8, 0.5, 0.2),     # Orange-Brown
        'hand': (0.7, 0.2, 0.3),        # Dark Red
        'face': (1.0, 0.7, 0.7),        # Light Pink
        'clothing': (0.0, 0.5, 0.7),    # Blue-Teal
        'shoes': (0.4, 0.3, 0.2),       # Brown
    }
    ...
    for i, (mask, label) in enumerate(zip(masks, labels)):
        ...
        # Get color based on class name or assign default
        if class_name in color_map:
            color = color_map[class_name]
        else:
            # Generate a color based on the index
            hue = (i * 0.618033988749895) % 1.0  # Golden ratio distribution
            saturation = 0.7
            value = 0.9
        ...
        if contour_method == 'skimage':
            contours = extract_contours_skimage(mask_uint8, simplify_tolerance)
        elif contour_method == 'cv2':
            # Use OpenCV for contours (alternative implementation)
            ...
            for cnt in cv_contours:
                ...
                if simplify_tolerance > 0:
                    epsilon = simplify_tolerance * cv2.arcLength(cnt, True) / 100
                    cnt = cv2.approxPolyDP(cnt, epsilon, True)
                # Convert to correct format
                contours.append(cnt.reshape(-1, 2))
        else:  # 'both' - use skimage (better quality for SVG)
            contours = extract_contours_skimage(mask_uint8, simplify_tolerance)
        
        # For maximum quality, use finer simplification
        if svg_quality == 'maximum' and simplify_tolerance > 0.03:
            contours = extract_contours_skimage(mask_uint8, simplify_tolerance / 2)
        ...
        # If no contours were found, create a simple bounding box
        if not geometric_elements:
            ...
            if len(y_indices) > 0 and len(x_indices) > 0:
                ...
                # Add rectangle as geometric element
                geometric_elements.append({
                    'type': 'rectangle',
...
def save_mask_as_png(mask, output_path, class_name, color):
    ...
    # Convert HTML color to CSS format if provided
    if isinstance(color, tuple) and len(color) == 3:
        # Already in RGB format (0-1)
        color_rgb = tuple(int(c * 255) for c in color)
    else:
        # Assume HTML color code like '#FF5733'
        color = color.lstrip('#')
...
def inference_detector(model,
                       ...
                       detailed_refinement=False,
                       edge_refinement=False,
                       expand_box_ratio=0.1,
                       multiscale_sam=False,
                       svg_quality='standard',
                       verify_export=False,
                       verification_report=False,
                       show_gpu_stats=False):
    """
    Main function to run detection and segmentation on an image.
    """
    # Read the image
    ...
    # Check if SAM predictor is initialized
if sam_predictor is None:
    ...
    try:
        ...
        # Process each box to get masks
        if len(detections.xyxy) > 0:
            if show_gpu_stats:
                print_gpu_memory(prefix="Before segmentation: ")
            
            # Process each detection
            for i, box in enumerate(detections.xyxy):
                print(f"Processing object {i+1}/{len(detections.xyxy)}: {labels[i]}")
                
                # Expand box to capture more context if requested
                if expand_box_ratio > 0:
                    box = expand_box(box, expand_box_ratio, image_rgb.shape[:2])
                
                try:
                    # Ensure box is valid
                    x1, y1, x2, y2 = box
                    if x2 <= x1 or y2 <= y1 or x1 >= image_rgb.shape[1] or y1 >= image_rgb.shape[0]:
                        print(f"  ⚠️ Invalid box dimensions: {box}")
                        box_mask = np.zeros(image_rgb.shape[:2], dtype=np.uint8)
                        all_masks.append(box_mask)
                        continue
                        
                    # Direct approach - pass box to SAM
                    masks, _, _ = sam_predictor.predict(
                        point_coords=None,
                        point_labels=None,
                        box=np.array(box),
                    ...
                    # Use the first mask if multiple were returned
                    if masks.shape[0] > 0:
                        mask = masks[0]
                        mask_uint8 = mask.astype(np.uint8)
                        all_masks.append(mask_uint8)
                        ...
                        # Fallback to simple box mask
                        print(f"  ⚠️ SAM returned empty mask, using box mask instead")
                        box_mask = np.zeros(image_rgb.shape[:2], dtype=np.uint8)
                        x1, y1, x2, y2 = map(int, box)
                        box_mask[y1:y2, x1:x2] = 1
                        all_masks.append(box_mask)
            ...
            if show_gpu_stats:
                print_gpu_memory(prefix="After segmentation: ")
                torch.cuda.empty_cache()
                print_gpu_memory(prefix="After segmentation cache clear: ")
        else:
            all_masks = []
    except Exception as e:
    ...
    refined_masks = []
    # Add diagnostic info about masks
if all_masks:
    print(f"Generated {len(all_masks)} masks with the following coverage:")
    for i, mask in enumerate(all_masks):
        pixels = np.sum(mask)
        percent = (pixels / (mask.shape[0] * mask.shape[1])) * 100
        print(f"  Mask {i+1}: Non-zero pixels={pixels} ({percent:.2f}% of image)")
else:
    ...
    if all_masks:
        ...
        for i, mask in enumerate(all_masks):
            ...
            if use_crf or detailed_refinement:
                # Apply optimized mask refinement
                refined = optimized_mask_refinement(image_rgb, processed_mask, refinement_mode)
                
                # Apply additional smoothing if requested
                if smooth_mask:
                    print(f"  Smoothing mask...")
                    refined = smooth_mask(refined, smooth_kernel_size)
                    
                refined_masks.append(refined)
            else:
                # Simple smoothing if no advanced refinement is requested
                if smooth_mask:
                    print(f"  Smoothing mask...")
                    smoothed = smooth_mask(processed_mask, smooth_kernel_size)
                    refined_masks.append(smoothed)
                else:
                    refined_masks.append(processed_mask)
    else:
        refined_masks = all_masks
    ...
    cv2.imwrite(os.path.join(output_dir, f"{name_without_ext}_yolo.jpg"), yolo_vis)
    cv2.imwrite(os.path.join(output_dir, f"{name_without_ext}_segmentation.jpg"), seg_vis)
    cv2.imwrite(os.path.join(output_dir, f"{name_without_ext}_combined.jpg"), combined_vis)
    ...
    if export_svg and refined_masks:
        ...
        if len(segments_data) > 0:
            ...
            # Add explicit message about SVG location
            if success:
            ...
            if success and verify_export and len(refined_masks) > 0:
                # Use first mask for verification
                verification_image_path = os.path.join(svg_dir, f"{name_without_ext}_verification.png")
            ...
            print(f"\n⚠️ No segments available for SVG export. Check segmentation quality.")
    
    # Export individual PNG masks if requested
    if export_png and refined_masks:
        png_dir = os.path.join(output_dir, "png_masks")
    ...
    # Save masks if annotation is enabled
    if annotation and refined_masks:
        annotation_dir = os.path.join(output_dir, "annotations")
        os.makedirs(annotation_dir, exist_ok=True)
        
        # Save each mask
        for i, mask in enumerate(refined_masks):
            mask_filename = os.path.join(annotation_dir, f"{name_without_ext}_mask_{i}.png")
            cv2.imwrite(mask_filename, (mask * 255).astype(np.uint8))
        
        # Save as YOLO format
        images_dict = {basename: image_rgb}
        ...
        annotations_dict = {basename: detections_with_masks}
        ...
        try:
            sv.DetectionDataset(
                classes=[text[0] for text in texts], 
                images=images_dict,
...
def main():
    ...
    # Initialize U²-Net
    u2net_predictor = None
    if hasattr(args, 'u2net_checkpoint') and args.u2net_checkpoint:
        try:
    ...
    crf_params = {
        ...
        'bi_w': args.crf_bi_w,
        'bi_xy': args.crf_bi_xy,
        'bi_rgb': args.crf_bi_rgb
    }
    ...
    if quality_features:
        print(f"Quality enhancements: {', '.join(quality_features)}")
...
if __name__ == '__main__':
    # Parse arguments and handle command line options
    if len(sys.argv) > 1 and sys.argv[1] == "--help":
        print("""
╔════════════════════════════════════════════════════════════════════════════╗
║              YOLO-World + SAM + U²-Net SVG Exporter Help                   ║
╠════════════════════════════════════════════════════════════════════════════╣
║ Usage:                                                                     ║
║   python sam_yolo_pro_export.py CONFIG CHECKPOINT IMAGE "TEXT" [OPTIONS]   ║
║                                                                            ║
║ Examples:                                                                  ║
║   Basic usage:                                                             ║
║     python sam_yolo_pro_export.py configs/yolo_world_v2_x.py               ║
║            weights/yolo_world_v2_x.pth demo/image.jpg "person, chair"      ║
║                                                                            ║
║   With SVG export and CRF refinement:                                      ║
║     python sam_yolo_pro_export.py configs/yolo_world_v2_x.py               ║
║            weights/yolo_world_v2_x.pth demo/image.jpg "person, chair"      ║
║            --export-svg --use-crf --svg-quality professional               ║
║                                                                            ║
║   For best quality results:                                                ║
║     python sam_yolo_pro_export.py configs/yolo_world_v2_x.py               ║
║            weights/yolo_world_v2_x.pth demo/image.jpg "person, chair"      ║
║            --export-svg --use-crf --detailed-refinement                    ║
║            --edge-refinement --svg-quality maximum                         ║
║            --u2net-checkpoint checkpoints/u2net.pth                        ║
║                                                                            ║
║ Main Arguments:                                                            ║
║   CONFIG                  Path to YOLO-World config file                   ║
║   CHECKPOINT              Path to YOLO-World model weights                 ║
║   IMAGE                   Path to input image or directory                 ║
║   TEXT                    Object categories to detect (comma-separated)    ║
║                                                                            ║
║ Key Options:                                                               ║
║   --sam-checkpoint PATH   Path to SAM checkpoint file                      ║
║   --u2net-checkpoint PATH Path to U²-Net checkpoint file                   ║
║   --export-svg            Export results as SVG files                      ║
║   --export-png            Export individual masks as PNG files             ║
║   --use-crf               Apply CRF refinement to masks                    ║
║   --detailed-refinement   Enable advanced mask refinement                  ║
║   --edge-refinement       Enable edge-aware refinement                     ║
║   --expand-box VALUE      Expand detection boxes (default: 0.1)            ║
║   --svg-quality LEVEL     Set SVG quality (standard/professional/maximum)  ║
║                                                                            ║
║ For more options, see:                                                     ║
║   python sam_yolo_pro_export.py --full-help                               ║
╚════════════════════════════════════════════════════════════════════════════╝
""")
        sys.exit(0)
    elif len(sys.argv) > 1 and sys.argv[1] == "--version":
        print(f"""
YOLO-World + SAM + U²-Net Professional SVG Exporter v{SCRIPT_VERSION}
Date: {SCRIPT_DATE}
Author: {SCRIPT_AUTHOR}
...
Environment:
  ...
  - CUDA: {"Not available" if not torch.cuda.is_available() else torch.version.cuda}
...
""")
        ...
        try:
            from segment_anything import sam_model_registry, SamPredictor
            print("Segment Anything (SAM) module: Available")
        except:
            print("Segment Anything (SAM) module: Not found")
            
        # Check for model files
        sam_checkpoint = "checkpoints/sam_vit_h_4b8939.pth"
        u2net_checkpoint = "checkpoints/u2net.pth"
        print(f"SAM checkpoint: {'Found' if os.path.exists(sam_checkpoint) else 'Not found'}")
        print(f"U²-Net checkpoint: {'Found' if os.path.exists(u2net_checkpoint) else 'Not found'}")

        print("\nSetup instructions:")
        print("""
1. Install dependencies:
   pip install torch torchvision opencv-python numpy matplotlib supervision
   pip install pydensecrf scikit-image svglib reportlab
   
2. Install MMCV and MMDetection:
...
4. Download model checkpoints:
   ...
   wget -P checkpoints/ https://huggingface.co/whosawhatsis/u2net/resolve/main/u2net.pth
   
   # Download YOLO-World model
   wget -P weights/ https://github.com/AILab-CVC/YOLO-World/releases/download/v0.2.0/yolo_world_v2_x_obj365v1_goldg_cc3mlite_pretrain_1280ft-14996a36.pth
   
5. Run the script:
   python sam_yolo_pro_export.py configs/pretrain/yolo_world_v2_x_vlpan_bn_2e-3_100e_4x8gpus_obj365v1_goldg_train_lvis_minival.py weights/yolo_world_v2_x_obj365v1_goldg_cc3mlite_pretrain_1280ft-14996a36.pth demo/sample_images/tenue.png "person, mask, glasses,hand,face, clothing, shoes" --export-svg --use-crf --detailed-refinement --edge-refinement --svg-quality maximum --sam-checkpoint checkpoints/sam_vit_h_4b8939.pth --u2net-checkpoint checkpoints/u2net.pth
""")
main()